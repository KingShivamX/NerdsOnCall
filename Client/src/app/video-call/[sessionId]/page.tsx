...
export default function VideoCallPage() {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden && isInCallRef.current) {
        console.log(" Page hidden during call, user may have switched tabs");
  ...
  useEffect(() => {
    if (localStreamRef.current) {
      ...
      if (videoTrack && videoTrack.enabled !== isVideoEnabled) {
        console.log(`🔧 Fixing video track: setting to ${isVideoEnabled}`);
        videoTrack.enabled = isVideoEnabled;
      }

      if (audioTrack && audioTrack.enabled !== isAudioEnabled) {
        console.log(`🔧 Fixing audio track: setting to ${isAudioEnabled}`);
    ...
    try {
      ...
      socketRef.current.onclose = () => {
        ...
        if (
          ...
          setTimeout(() => {
            if (isInCallRef.current && !isEndingSessionRef.current) {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case "call_accepted":
          ...
          break;
        case "call_declined":
          console.log("📞❌ Call declined by teacher - Full message:", message);
          ...
          const executeCallDeclineCleanup = async () => {
            console.log("🚨 EXECUTING CALL DECLINE CLEANUP - GUARANTEED");

            try {
              // Cancel the session in backend to prevent billing
              console.log("🚫 Cancelling session due to call decline...");
              await api.put(
            ...
            console.log("🔙 REDIRECTING BACK DUE TO CALL DECLINE - GUARANTEED");
      ...
      if (localStreamRef.current && peerConnectionRef.current) {
        localStreamRef.current.getTracks().forEach((track) => {
          if (peerConnectionRef.current && localStreamRef.current) {
            ...
            if (track.kind === "video") {
              track.enabled = isVideoEnabled;
              console.log(`📹 Video track enabled for call: ${track.enabled}`);
            } else if (track.kind === "audio") {
              track.enabled = isAudioEnabled;
              console.log(`🎤 Audio track enabled for call: ${track.enabled}`);
            }
  ...
  const handleOtherUserEndCall = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.warn("Session end failed (other user initiated):", error);
        // Don't show duplicate message since we already showed one above
        console.log(
  ...
  const handleDeclineCall = useCallback(async () => {
    ...
    if (socketRef.current && incomingCall) {
      ...
      try {
        console.log("🚫 Cancelling session due to call decline by teacher...");
        await api.put(
              ...
              <div className="bg-green-300 text-black p-2 flex items-center justify-between border-b-4 border-black react-grid-draghandle cursor-move">
                ...
                <div className="flex items-center space-x-2">
                  {focusMode === "whiteboard" ? (
                    ...
                    <Button
                      onClick={() => enterFocusMode("whiteboard")}
