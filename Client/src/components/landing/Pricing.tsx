...
export function Pricing() {
    ...
    useEffect(() => {
        const fetchSubscription = async () => {
            if (user && user.role === "STUDENT") {
                try {
                    const res = await api.get<Subscription | string>("/subscriptions/my-subscription")
                    if (typeof res.data === "object" && res.data !== null && "id" in res.data) {
            ...
            const options = {
                ...
                handler: async function (rzpResponse: any) {
                    try {
                        ...
                        const subRes = await api.get<Subscription | string>("/subscriptions/my-subscription");
                        if (typeof subRes.data === "object" && subRes.data !== null && "id" in subRes.data) {
    ...
    if (loading) {
        return (
            <div className="py-20 text-center text-lg text-slate-600">Loading plans...</div>
        )
    }
    if (error) {
        return (
                ...
                {user && user.role === "STUDENT" && subscription && (
                    <div className="mb-8 text-center">
                        <div className="inline-block bg-emerald-100 text-emerald-800 px-4 py-2 rounded-full font-semibold">
                            You are subscribed to: <b>{subscription.planType}</b> (Status: {subscription.status})
                        </div>
                    </div>
                )}
                            ...
                            >
                                ...
                                <CardContent className="pt-0 px-4 sm:px-6 pb-6 sm:pb-8">
                                    ...
                                    {user && user.role === "STUDENT" ? (
                                        ...
                                        >
                                            {payingPlanId === plan.id ? "Processing..." : "Subscribe Now"}
                                        </Button>
                                    ) : (
                                        <Link href={`/auth/register?role=student&plan=${plan.name.toLowerCase()}`}>
                                            <Button
                                                className={`w-full h-10 sm:h-12 font-semibold transition-all duration-200 text-sm sm:text-base ${
                                                    isPopular(plan)
                                                        ? "bg-slate-800 hover:bg-slate-900 focus:bg-slate-900 active:bg-slate-950 text-white hover:shadow-lg focus:ring-2 focus:ring-slate-400 focus:ring-offset-2"
                                                ...
                                                variant={isPopular(plan) ? "default" : "outline"}
                                            >
                                                {isPopular(plan) ? "Get Started" : "Start Free Trial"}
                                            </Button>
                                        </Link>
                                    )}
                                </CardContent>
