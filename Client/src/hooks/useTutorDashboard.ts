...
export function useTutorDashboard() {
    ...
    const fetchDashboardData = async () => {
        ...
        if (!user) {
            console.log('⏳ User not loaded yet, skipping tutor dashboard fetch')
            setLoading(false)
            return
        }

        if (user.role !== 'TUTOR') {
            console.log('👨‍🎓 User is not a tutor, skipping tutor dashboard fetch')
            setLoading(false)
            return
        }
        ...
        try {
            setLoading(true)
            setError(null)
            console.log('📊 [TUTOR] Fetching dashboard data for tutor:', user.firstName, user.lastName)
            ...
            const errorMessage = err.response?.data || err.message || 'Failed to fetch tutor dashboard data'
            ...
            console.warn('Tutor dashboard data fetch failed, using default values:', errorMessage)
        } finally {
            setLoading(false)
        }
    }
